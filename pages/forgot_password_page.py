"""
Page Object для страницы восстановления пароля Stellar Burgers
"""
from pages.base_page import BasePage
from locators.forgot_password_page_locators import ForgotPasswordPageLocators


class ForgotPasswordPage(BasePage):
    """Класс для работы со страницей восстановления пароля"""

    def __init__(self, driver):
        super().__init__(driver)

    def wait_for_page_load(self):
        """Ожидает загрузки страницы восстановления пароля"""
        # Ожидаем появления заголовка страницы
        self.wait_for_element_present(ForgotPasswordPageLocators.PAGE_TITLE)

    def click_login_link(self):
        """Кликает на ссылку 'Войти'"""
        login_link = self.wait_for_element_clickable(ForgotPasswordPageLocators.LOGIN_LINK)
        login_link.click()
